...
class DashboardGenerator {
  ...
  generateCSS() {
    ...
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .sparkline-container{position:absolute;bottom:0;right:0;width:120px;height:40px;opacity:0.6}
    ...
    @media print{:root{--bg:#fff;--surface:#fff;--text:#1e293b;--muted:#64748b;--border:#e2e8f0}.nav,.pivot-controls{display:none}.page{display:block!important}}
    `;
  }
  ...
  generateHTML() {
    ...
    const byService = Object.entries(this.unitEconomics?.byService || {}).slice(0, 8);
...
<script>${this.generateJS(metrics, topAccounts, historical, esr, byService)}</script>
</div></body></html>`;
  }
  ...
  <div class="page active" id="overview">
    <div class="grid grid-5">
      ...
      <div class="card kpi-card">
        <div class="kpi-label">Effective Savings Rate</div>
    ...
    <div class="card">
      <div class="section-title">📊 Account Breakdown <span class="drill-toggle" onclick="toggleDrill('acct-drill')">[expand details]</span></div>
      <table>
  ...
  <div class="page" id="kpis">
    <div class="card" style="margin-bottom:20px;background:linear-gradient(135deg,rgba(99,102,241,0.1),rgba(74,222,128,0.05));border-color:rgba(99,102,241,0.3)">
      <div class="grid" style="grid-template-columns:1fr 1fr 1fr;gap:24px;align-items:center">
        ...
        <div>
          <div class="kpi-label">Monthly RI/SP Savings</div>
    ...
    <div class="card">
      ...
      <table id="service-table">
        <tr><th>Service</th><th>Amortized Cost</th><th>Cost/vCPU Hr</th><th>ESR</th><th>Savings</th></tr>
        ${byService.map(([svc, d]) => `<tr>
          <td><strong>${svc}</strong></td>
          <td>${this.fmt(d?.current?.amortized?.totalCost ?? 0)}</td>
          <td>$${(d?.current?.amortized?.costPerVCPUHour ?? 0).toFixed(4)}</td>
    ...
    <div class="card" style="margin-top:16px">
      <div class="section-title">🖥️ Instance Type Analysis <span class="drill-toggle" onclick="toggleDrill('instance-drill')">[show all]</span></div>
      <table>
  ...
  <div class="page" id="insights">
    ...
    <div class="pivot-controls">
      <button class="pivot-btn active" onclick="filterActions('all')">All</button>
    ...
    ${actions.slice(0, 12).map(a => `
      <div class="card insight-card ${a.priority}" data-priority="${a.priority}">
    ...
    <div class="grid grid-3" style="margin-top:20px">
      ...
      <div class="card">
        <div class="section-title">🔗 Cross-Metric Patterns</div>
  ...
  <div class="page" id="optimization">
    <div class="grid grid-3" style="grid-template-columns:1fr 1fr 1fr">
      <div class="card kpi-card">
        <div class="kpi-label">Total Savings Available</div>
    ...
    <div class="card">
      <div class="section-title">🎯 Top Rightsizing Recommendations
        <div class="pivot-controls" style="margin:0;margin-left:auto">
      ...
      <table id="rightsizing-table">
        <tr><th>Resource</th><th>Account</th><th>Current</th><th>Recommended</th><th>Savings %</th><th>Monthly $</th></tr>
        ${rightsizing.map(r => `<tr data-savings="${r.monthlySavings || 0}" data-pct="${r.savingsPercentage || 0}">
  ...
  <div class="page" id="anomalies">
    <div class="grid grid-3" style="grid-template-columns:1fr 1fr 1fr">
      ...
      <div class="card kpi-card">
        <div class="kpi-label">Largest Spike</div>
    ...
    <div class="card">
      <div class="section-title">🚨 Anomaly Timeline
        <div class="pivot-controls" style="margin:0;margin-left:auto">
      ...
      <table id="anomaly-table">
        <tr><th>Date</th><th>Account</th><th>Service</th><th>Increase</th><th>Cost</th><th>Severity</th></tr>
        ${anomalies.map(a => {
  ...
  generateJS(metrics, topAccounts, historical, esr, byService) {
    // Prepare chart data from historical months
    ...
    function filterActions(priority) {
      document.querySelectorAll('.pivot-controls .pivot-btn').forEach(b => b.classList.remove('active'));
      event.target.classList.add('active');
    ...
    function filterAnomalies(filter) {
      ...
      event.target.classList.add('active');
      document.querySelectorAll('#anomaly-table tr[data-severity]').forEach(row => {
    ...
    function sortRightsizing(by) {
      event.target.parentElement.querySelectorAll('.pivot-btn').forEach(b => b.classList.remove('active'));
      event.target.classList.add('active');
      const table = document.getElementById('rightsizing-table');
      const rows = Array.from(table.querySelectorAll('tr[data-savings]'));
    ...
    function pivotAccounts(mode) {
      event.target.parentElement.querySelectorAll('.pivot-btn').forEach(b => b.classList.remove('active'));
      event.target.classList.add('active');
      // Rebuild chart with different sort - placeholder for dynamic pivot
    }
